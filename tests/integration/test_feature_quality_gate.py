import pytest
import asyncio
from orchestrator import do_save_async, do_get_async, do_get_verification_status
from core.exceptions import ValidationError, SyntaxValidationError
...
async def test_integration_save_valid_function(test_db):
    ...
    accepted = await do_save_async(
        name=name, code=code, description="integration test",
        test_code=test_code, project="integ"
    )
...
async def test_integration_save_syntax_error(test_db):
    ...
    with pytest.raises(SyntaxValidationError):
        await do_save_async(
            name=name, code=code, test_code="", project="integ"
        )
...
async def test_integration_draft_mode(test_db):
    ...
    assert status_data["status"] in ["failed", "error"] # A draft without a valid test fails verification but remains in the vault
