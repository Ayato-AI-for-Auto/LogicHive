...
async def test_chaos_complexity_ceiling():
    ...
    results = await manager.evaluate_all(
        code=code, language="python", test_code="assert complex_func(1) != 0"
    )
...
async def test_chaos_assertion_flooding_quality_theater():
    ...
    res_theater = await manager.evaluate_all(
        code=code, language="python", test_code=test_code_theater
    )
    assert res_theater["details"]["deterministic"]["details"]["assertion_count"] == 0
    print(
        f"DEBUG: Theater Score: {res_theater['details']['deterministic']['score']} (Expected: 0.0)"
    )
    ...
    print(
        f"DEBUG: Real Nested Score: {res_real['details']['deterministic']['score']} (Expected: 100.0)"
    )
