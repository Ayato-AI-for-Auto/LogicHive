import pytest
import asyncio
from mcp_server import save_function, search_functions, get_verification_status, get_function, delete_function
...
async def test_system_end_to_end_flow(test_db):
    ...
    save_msg = await save_function(
        name=name, code=code, description="Parses data to uppercase",
        tags=["parser", "string"], test_code=test_code, project="sys_test"
    )
