            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name.split(".")[0] in cls.FORBIDDEN_IMPORTS:
                            return (
                                False,
                                f"Security Block: Import of '{alias.name}' is forbidden.",
                            )

                elif isinstance(node, ast.ImportFrom):
                    if (
                        node.module
                        and node.module.split(".")[0] in cls.FORBIDDEN_IMPORTS
                    ):
                        return (
                            False,
                            f"Security Block: Import from '{node.module}' is forbidden.",
                        )
                ...
                # 2. Check Function Calls
                    ...
                    if isinstance(node.func, ast.Name):
                        ...
                        if (
                            func_name in cls.FORBIDDEN_CALLS
                            or func_name in cls.FORBIDDEN_BUILTINS
                        ):
                            return (
                                False,
                    ...
                    # Attribute calls: os.system(), subprocess.run()
                        ...
                        if method_name in cls.FORBIDDEN_CALLS:
                            return (
                                False,
                                f"Security Block: Attribute call '{method_name}' is forbidden.",
                            )
                ...
                # 3. Check for dunder methods/attributes access (e.g. __class__)
                    ...
                    if node.attr.startswith("__") and node.attr.endswith("__"):
                        ...
                        if node.attr not in {"__name__", "__init__"}:
                            return (
                                False,
                                f"Security Block: Access to dunder attribute '{node.attr}' is forbidden.",
                            )
