...
if getattr(sys, "frozen", False):
    ...
    # Use environment variable FS_DATA_DIR or fallback to local 'data'
    # In Cloud Run, /tmp or a mounted volume should be used
...
def get_setting(key: str, default=None):
    """Gets a setting from env var first (SaaS/Cloud priority), then UI-saved settings.json, then default."""
    val = os.getenv(key)
    if val is not None:
        return val
...
HUB_URL = get_setting("FS_HUB_URL", "https://function-store-hub-wqrdbid6cq-an.a.run.app")
...
# Ollama Config
OLLAMA_BASE_URL = get_setting("FS_OLLAMA_BASE_URL", "http://localhost:11434")
OLLAMA_EMBED_MODEL = get_setting("FS_OLLAMA_EMBED_MODEL", "mxbai-embed-large")
OLLAMA_CHAT_MODEL = get_setting("FS_OLLAMA_CHAT_MODEL", "qwen2.5-coder:7b")
...
SYNC_REPO_URL = get_setting(
    "FS_SYNC_REPO_URL", "https://github.com/Ayato-AI-for-Auto/LogicHive-hub.git"
)
