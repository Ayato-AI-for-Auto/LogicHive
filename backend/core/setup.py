...
def generate_config(is_frozen=False):
    ...
    if is_frozen:
        ...
        command = "uv"
        args = ["run", "--with", "duckdb", "--with", "fastmcp", "python", "-m", "edge.mcp_server"]
    ...
    inject_targets = [
        ...
        Path(os.getenv("APPDATA", "")) / "Cursor/User/globalStorage/heavy.cursor.cursor/mcp_config.json",
    ...
    if not injected_any:
        ...
        print("Please manually add the contents of 'mcp_config_logic_hive.json' to your editor settings.")
    else:
    ...
    if os.name == "nt" and not is_frozen:
        ...
        with open(bat_path, "w") as f:
            ...
            f.write(f'uv run --with duckdb --with fastmcp python -m edge.mcp_server\n')
