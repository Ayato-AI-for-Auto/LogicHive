...
from mcp_core.core.vector_db import get_vector_db
        ...
        with DBWriteLock():
            ...
            try:
                ...
                if row:
                    ...
                    get_vector_db().upsert_function(fid, v_list, payload)
...
def _do_search_query(query: str, limit: int = 20) -> List[Dict]:
    ...
    # 4. Search using Qdrant (High Performance)
    search_results = get_vector_db().search(query_embedding, limit=limit)
        ...
        results.append(
            {
                ...
                "score": round(
                    float(point.score) * 0.7
                    + (p.get("quality_score", 50) / 100.0) * 0.3,
                    4,
                ),
            }
        )
...
def do_archive_impl(asset_name: str) -> str:
    """Core logic for archiving a function (soft delete) and removing its vectors."""
    with DBWriteLock():
        conn = get_db_connection()
        try:
            row = conn.execute(
            ...
            if row:
                ...
                conn.execute("DELETE FROM embeddings WHERE function_id = ?", (fid,))
                conn.execute(
                    "UPDATE functions SET status = 'archived', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (fid,),
                )
                ...
                conn.commit()
                return f"SUCCESS: Function '{asset_name}' archived and its vector data removed."
            return f"Error: Function '{asset_name}' not found."
        except Exception as e:
            logger.error(f"Archive Error: {e}")
            return f"Error: Failed to archive function '{asset_name}': {e}"
        finally:
            conn.close()


def do_restore_impl(asset_name: str) -> str:
    """Core logic for restoring an archived function and queuing it for re-verification."""
    with DBWriteLock():
        conn = get_db_connection()
        try:
            row = conn.execute(
                "SELECT id, code, description, tags, metadata, test_cases FROM functions WHERE name = ?",
                (asset_name,),
            ).fetchone()
            if row:
                fid, code, description, tags_json, metadata_json, tests_json = row
                ...
                conn.execute(
                    "UPDATE functions SET status = 'unverified', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (fid,),
                )
                conn.commit()
            else:
            ...
            return f"Error: Failed to restore function '{asset_name}': {e}"
        finally:
            conn.close()

    # Trigger background maintenance to re-vectorize and verify (outside of DBWriteLock)
    task_worker.add_task(
        run_background_maintenance,
        asset_name,
        code,
    ...
    return f"SUCCESS: Function '{asset_name}' restored and queued for re-verification."
...
def do_get_details_impl(name: str) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "call_count": row[5] + 1,  # Return the updated count
...
def do_smart_get_impl(query: str, target_dir: str = "./") -> Dict:
    ...
    external_deps = details.get("metadata", {}).get("dependencies", [])
    ...
    return {
        ...
        "recommendation": f"Run 'uv pip install {' '.join(external_deps)}' if you encounter import errors."
        if external_deps
        else "No external dependencies required.",
    }
