...
class Coordinator:
    ...
    def start_master_invisible(self):
        ...
        for _ in range(30):  # Increase timeout for robustness
            if self.is_master_running():
                # Verify it's actually responding to HTTP
                try:
                    with httpx.Client(timeout=1.0) as client:
                        client.get(f"{self.master_url}/execute", timeout=1.0)
                        # Root or /execute might 404 on GET, but connection means it's up.
                        # Better check: background_server.py should have a health endpoint.
                        logger.info("Coordinator: Master process is now responding.")
                        return
                except Exception:
                    pass
            time.sleep(0.5)
