...
2.  **ステートフルなデータ管理**: ユーザーのPCのストレージを利用し、DuckDBを用いたローカルプロジェクト固有のキャッシュを管理する（必要時のみ）。
3.  **シームレスなクラウド連携**: Globalな関数インテリジェンス（Supabase）を管理するGCP上のHubと通信し、即座に関数のセマンティック検索や保存（Push）を行う。
...
### 2.3 Hub Communication Engine
*   **役割**: Cloud Intelligence Hub (GCP) との間で直接REST通信を行い、関数のセマンティック検索やPushを代行する。
*   **実装**: 従来の同期的なGitストレージ（`LogicHive-Storage`）を完全に廃止し、Supabaseを利用したリアルタイムのベクトル検索インターフェースへと移行。これによりローカルでの重い同期処理やコンフリクト解決が不要となった。

## 3. Reverse Intelligence Flow (Edge側の視点)

ハイブリッドな検索と取得（`smart_search_and_get`）を行う際のシーケンス：

1.  **Global Semantic Search**: `ts-proxy` (またはPython Edgeクライアント) は、GCP Hub に対してユーザーの自然言語クエリを送信し、Hub側のSupabase (pgvector) から最適な候補（Candidates）を瞬時に取得する。
