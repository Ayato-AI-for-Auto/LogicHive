...
cur.execute(
    "SELECT COUNT(*) FROM logichive_functions WHERE embedding IS NOT NULL AND embedding != '' AND embedding != '[]'"
)
...
cur.execute(
    "SELECT name, project, CASE WHEN embedding IS NULL THEN 'NULL' WHEN embedding = '' THEN 'EMPTY' WHEN embedding = '[]' THEN 'BRACKETS' ELSE 'HAS_EMBEDDING(' || LENGTH(embedding) || ')' END as status FROM logichive_functions ORDER BY status, name"
)
