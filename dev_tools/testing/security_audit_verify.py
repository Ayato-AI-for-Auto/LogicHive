...
def test_security_audit():
    ...
    with patch("git.Repo") as mock_repo_class, patch("httpx.Client.post") as mock_post:
        # Mock Git
        mock_repo = MagicMock()
        ...
        (Path(os.environ["FS_SYNC_LOCAL_DIR"]) / ".git").mkdir(
        ...
        if mock_post.called:
            ...
            if secret_key in payload_str or secret_key in str(
                kwargs.get("headers", {})
            ):
                print(
                    "[FAILURE] 🚨 SECURITY BREACH: Secret API Key found in outgoing payload!"
                )
                exit(1)
            else:
            ...
            print(
                "[ERROR] Push operation was not called. Check if ensure_repo() passed."
            )
            exit(1)
