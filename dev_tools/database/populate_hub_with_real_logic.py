...
def scan_python_ast_security(code: str) -> Tuple[bool, str]:
    ...
    FORBIDDEN_CALLS = {"eval", "exec", "compile", "breakpoint", "__import__", "system", "popen", "spawn", "fork", "kill"}
    FORBIDDEN_BUILTINS = {"open", "getattr", "setattr", "delattr", "hasattr", "globals", "locals", "vars", "dir", "help", "input"}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if isinstance(node.func, ast.Name):
                    if node.func.id in FORBIDDEN_CALLS or node.func.id in FORBIDDEN_BUILTINS:
...
def run_shell_cmd_with_timing(cmd: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": str(e), "elapsed_seconds": round(time.time() - start, 4)}

# --- 5. cleanup_temp_files ---
import os
def cleanup_temp_files(directory: str, extensions: List[str] = [".log", ".tmp"]) -> Dict[str, Any]:
    """
